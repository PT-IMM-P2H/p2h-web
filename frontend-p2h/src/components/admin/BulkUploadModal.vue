<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div class="px-6 py-4">
        ...
        <div class="mb-6">
          ...
          >
            ...
            <svg v-if="!selectedFile" class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="uploadResult" class="mb-6">
          <div :class="[
            ...
            uploadResult.error_count === 0 ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'
          ]">
            <div class="flex items-start gap-3">
              <svg v-if="uploadResult.error_count === 0" class="h-6 w-6 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <svg v-else class="h-6 w-6 text-yellow-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <div class="flex-1">
                <h4 :class="['font-semibold mb-2', uploadResult.error_count === 0 ? 'text-green-900' : 'text-yellow-900']">
                  Import Selesai
                </h4>
                <div class="text-sm space-y-1">
                  <p :class="uploadResult.error_count === 0 ? 'text-green-800' : 'text-yellow-800'">
                    ✅ Berhasil: <strong>{{ uploadResult.success_count }}</strong> dari {{ uploadResult.total_rows }} baris
                  </p>
                  <p v-if="uploadResult.error_count > 0" class="text-yellow-800">
                    ⚠️ Error: <strong>{{ uploadResult.error_count }}</strong> baris dilewati
                  </p>
                </div>
                ...
                <div v-if="uploadResult.errors && uploadResult.errors.length > 0" class="mt-3">
                  ...
                  >
                    ...
                    <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showErrors }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
                  
                  <div v-if="showErrors" class="mt-2 max-h-48 overflow-y-auto">
                    <div class="bg-white rounded border border-yellow-300 divide-y divide-yellow-200">
                      <div
                        v-for="(error, idx) in uploadResult.errors.slice(0, 20)"
                        :key="idx"
                        class="px-3 py-2 text-xs"
                      >
                        <span class="font-semibold text-gray-900">Baris {{ error.row }}:</span>
                        <span class="text-gray-700 ml-1">{{ error.message }}</span>
                        <span v-if="error.field" class="text-gray-500 ml-1">(Field: {{ error.field }})</span>
                      </div>
                      <div v-if="uploadResult.errors.length > 20" class="px-3 py-2 text-xs text-gray-600 italic">
      ...
      <!-- Footer -->
      <div class="sticky bottom-0 bg-gray-50 px-6 py-4 border-t border-gray-200 rounded-b-lg flex justify-end gap-3">
        <button
          @click="$emit('close')"
...
const downloadTemplate = async () => {
  try {
    ...
    const response = await axios.get(props.templateEndpoint, {
      headers: {
